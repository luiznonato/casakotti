...
<div class="wrap ckf-admin">
	<h1><?php esc_html_e( 'Questionário', 'casa-kotti-feedback' ); ?></h1>
	...
	foreach ( $steps as $step ) :
		...
		<section class="ckf-page-card" draggable="true" data-step-id="<?php echo esc_attr( (string) $step->id ); ?>">
			...
			<header>
				...
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=casa-kotti-questions&step=' . absint( $step->id ) ) ); ?>"><?php esc_html_e( 'Editar página', 'casa-kotti-feedback' ); ?></a>
					|
					<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=ckf_reorder_step&id=' . absint( $step->id ) . '&dir=up' ), 'ckf_reorder_step_' . absint( $step->id ) ) ); ?>">↑</a>
					<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=ckf_reorder_step&id=' . absint( $step->id ) . '&dir=down' ), 'ckf_reorder_step_' . absint( $step->id ) ) ); ?>">↓</a>
					|
					<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=ckf_duplicate_step&id=' . absint( $step->id ) ), 'ckf_duplicate_step_' . absint( $step->id ) ) ); ?>"><?php esc_html_e( 'Duplicar página', 'casa-kotti-feedback' ); ?></a>
					|
