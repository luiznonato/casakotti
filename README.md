...
- `plugin/mocori-insights/`: plugin instalável **Mocori Insights** (produto Mocori, client-agnostic).
...
1. Em **Plugins > Adicionar plugin > Enviar plugin**, envie `dist/mocori-insights.zip` e ative **Mocori Insights** se for medir visitantes e conversões.
2. Envie `dist/casa-kotti-interesses.zip` e ative.
3. Envie `dist/casa-kotti-feedback.zip` e ative **Casa Kotti — Avaliações** se for usar o questionário de experiência (`[casa_kotti_feedback]`). Detalhes em `plugin/casa-kotti-feedback/README.md`.
4. Em **Aparência > Temas > Adicionar tema > Enviar tema**, envie `dist/casa-kotti.zip` e ative quando estiver pronto para substituir o tema atual.
5. Crie uma página (por exemplo, “Início”) e selecione-a em **Configurações > Leitura** como página inicial estática.
6. Crie e publique a política de privacidade aprovada. Selecione a mesma página:
   - em **Configurações > Privacidade**; e
   - em **Aparência > Personalizar > Casa Kotti — Pré-lançamento**.
7. Em **Aparência > Personalizar**, confirme o logo e configure textos, links, contato e formulário. O logo oficial já acompanha o tema; um logo configurado no WordPress continua tendo prioridade.
...
```bash
rm -f dist/casa-kotti.zip dist/casa-kotti-interesses.zip dist/casa-kotti-feedback.zip dist/mocori-insights.zip
...
(cd plugin && zip -qr ../dist/mocori-insights.zip mocori-insights -x 'mocori-insights/tests/*')
```
...
- [`preview/frame-6x9.png`](preview/frame-6x9.png) (1024 × 1536, quadro 6×9)
